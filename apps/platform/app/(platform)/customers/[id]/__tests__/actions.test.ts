...
vi.mock('@roaring/db', () => ({ db: mockDb, customers: {} }))
...
describe('updateCustomer', () => {
  ...
  it('calls requireUser to gate the action', async () => {
    await updateCustomer('cust-1', baseData)
  ...
  it('revalidates the customers path', async () => {
    await updateCustomer('cust-1', baseData)
  ...
  it('propagates requireUser errors without running the transaction', async () => {
    ...
    await expect(updateCustomer('cust-1', baseData)).rejects.toThrow('Unauthorized')
