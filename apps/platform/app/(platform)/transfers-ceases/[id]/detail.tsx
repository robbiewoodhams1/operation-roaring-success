...
import { updateTransferCease, updateTransferCeaseStatus } from '../actions'
import type { TransferCease, TransferCeaseComment } from '@roaring/db'
import {
...
export function Detail({
  ...
  record: TransferCease
  comments: TransferCeaseComment[]
  ...
  async function handleSave() {
    setSaving(true)
    await updateTransferCease(record.id, {
      type: form.type,
  ...
  async function handleAddComment() {
    ...
    await addComment(record.id, commentBody.trim())
    setCommentBody('')
  ...
  async function handleDeleteComment(commentId: string) {
    await deleteComment(commentId, record.id)
    router.refresh()
  }
        ...
        <div className="divide-y">
          ...
          <Row label="Assigned to">
            {isEditing ? (
              ...
              <span className="text-sm">
                {record.assignedTo ? (userMap[record.assignedTo] ?? '—') : '—'}
              </span>
            )}
          </Row>
          <Row label="Opened">
            <span className="text-sm">{new Date(record.openedAt).toLocaleString('en-GB')}</span>
          </Row>
          {record.completedAt && (
            <Row label="Completed">
              <span className="text-sm">
